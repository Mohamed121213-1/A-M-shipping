...
export const PRIMARY_ADMIN_USER = {
  id: 'admin_root',
  ...
  try {
    ...
    if (Array.isArray(state?.couriers)) {
      if (state.couriers.length === 0) {
        try {
          await supabaseServer.from('couriers').delete().neq('id', '___none___');
        } catch (err) {}
      } else {
...
async function pullStateFromSupabaseOnBoot() {
  ...
  try {
    ...
    if (!pErr && Array.isArray(pRows) && pRows.length > 0) {
      const validProfiles = pRows.filter((p: any) => !isDeprecatedDummyUser(p));
      const mappedProfiles = validProfiles.map((p: any) => ({
        id: String(p.id),
        name: p.name || 'مستخدم',
    ...
    if (!sErr && Array.isArray(sRows) && sRows.length > 0) {
      const mapped = sRows.map((r: any) => {
        ...
        const currentRank = STATUS_RANK[base.status] || 0;
        const rowRank = STATUS_RANK[r.status] || 0;
        const effectiveStatus = (currentRank >= rowRank && currentRank > 0) ? base.status : (r.status || base.status || 'created');

        const currentName = (base.recipient?.name || '').trim();
        const rowName = (r.customer_name || '').trim();
        let safeName = 'عميل';
        if (currentName && !/^[\s.,'،_\-]*$/.test(currentName)) {
          safeName = currentName;
        } else if (rowName && !/^[\s.,'،_\-]*$/.test(rowName)) {
          safeName = rowName;
        } else {
          const city = r.city || base.recipient?.city || '';
          safeName = city ? `عميل (${city})` : `عميل (${r.customer_phone || base.recipient?.phone || 'معتمد'})`;
        }
...
app.delete("/api/shipments/:id", async (req, res) => {
  try {
    ...
    if (supabaseServer) {
      try {
        await supabaseServer.from('shipments').delete().or(`id.eq.${id},tracking_number.eq.${id}`);
      } catch (e) {}
    }
...
app.post("/api/shipments/batch-delete", async (req, res) => {
  try {
    const { ids, senderId } = req.body || {};
    if (Array.isArray(ids) && serverAppState && Array.isArray(serverAppState.shipments)) {
      ...
      if (supabaseServer && ids.length > 0) {
        try {
          await supabaseServer.from('shipments').delete().in('id', ids);
          await supabaseServer.from('shipments').delete().in('tracking_number', ids);
        } catch (e) {}
      }
    }
...
app.post("/api/sync/state", (req, res) => {
  try {
    ...
    if (!isExplicitClear && serverAppState) {
      ...
      if (Array.isArray(state.users) && state.users.length > 0) {
        mergedState.users = sanitizeServerState({ users: [...(serverAppState.users || []), ...state.users] }).users;
      } else if (Array.isArray(serverAppState.users) && serverAppState.users.length > 0) {
        mergedState.users = serverAppState.users;
      }

      // Wallet protection: prevent zeroing out active wallet balance from stale or uninitialized clients
      if (state.wallet) {
        const currentWallet = serverAppState.wallet;
        if (currentWallet && 
            (currentWallet.availableBalance > 0 || currentWallet.pendingCod > 0) &&
            (state.wallet.availableBalance === 0 && state.wallet.pendingCod === 0 && state.wallet.totalPaidOut === 0)) {
          console.warn('🛡️ Prevented zeroing out active wallet balance in sync');
          mergedState.wallet = currentWallet;
        } else {
          mergedState.wallet = { ...(currentWallet || {}), ...state.wallet };
        }
      } else if (serverAppState.wallet) {
        mergedState.wallet = serverAppState.wallet;
      }

      if (Array.isArray(state.couriers) && state.couriers.length > 0) {
        mergedState.couriers = sanitizeServerState({ couriers: [...(serverAppState.couriers || []), ...state.couriers] }).couriers;
