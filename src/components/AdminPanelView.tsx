...
export const AdminPanelView: React.FC<AdminPanelViewProps> = ({
  ...
  const handleSaveCourier = (e: React.FormEvent) => {
    ...
    setCourierFormData({ name: '', phone: '', vehicle: 'motocycle', assignedHub: hubs[0]?.name || '', rating: 5.0 });
  };
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-5">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                ...
                إدارة كباتن ومندوبي الشحن
              </h2>
              <p className="text-xs font-semibold text-slate-500">
                إضافة كباتن، ربطهم بالمستودعات، وتحديث وسيلة النقل ورقم الهاتف
              </p>
            </div>
            ...
            <button
              onClick={() => {
                setEditingCourier(null);
                setCourierFormData({ name: '', phone: '', vehicle: 'motocycle', assignedHub: hubs[0]?.name || 'المستودع الرئيسي', rating: 5.0 });
                setIsCourierModalOpen(true);
              }}
              className="bg-red-600 hover:bg-red-700 text-white font-bold text-xs px-4 py-2.5 rounded-xl shadow-sm transition-all flex items-center gap-2"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredCouriers.map((c) => (
              <div key={c.id} className="bg-slate-50 border border-slate-200 p-4 rounded-2xl flex items-start justify-between gap-3 hover:border-red-300 transition-all">
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-[10px] text-slate-500 font-semibold flex-wrap pt-1">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                ...
                تعديل أسعار الشحن وأيام التسليم حسب المحافظة
              </h2>
              <p className="text-xs font-semibold text-slate-500">
                يمكن للأدمن تخصيص أسعار الشحن الأساسية وسعر الكيلو الزائد لكافة المحافظات
              </p>
            </div>
            ...
            <table className="w-full text-right text-xs">
              <thead className="bg-slate-50 border-b border-slate-200 font-extrabold text-slate-600">
                <tr>
                  ...
                  <th className="p-3.5">السعر الأساسي (حتى 3 كجم)</th>
                  <th className="p-3.5">سعر الكيلو الإضافي</th>
                  <th className="p-3.5">مدة التسليم المتوقعة</th>
                  <th className="p-3.5 text-center">إجراءات التعديل</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 font-bold text-slate-800">
                {filteredRates.map((g) => (
                  <tr key={g.code} className="hover:bg-slate-50 transition-colors">
                    <td className="p-3.5">
                      <span className="font-black text-slate-900">{g.nameAr}</span>
                      <span className="text-[10px] text-slate-400 font-mono block">{g.nameEn}</span>
                    </td>
                    <td className="p-3.5 font-black text-red-600">
                      {g.baseRate} ج.م
                    </td>
                    <td className="p-3.5 text-slate-700">
                      +{g.additionalKgRate} ج.م / كجم
                    </td>
                    <td className="p-3.5">
                      <span className="bg-slate-100 text-slate-700 px-2 py-1 rounded text-[11px] font-bold">
                        {g.estDays}
                      </span>
                    </td>
                    <td className="p-3.5 text-center">
                      <button
                        ...
                        className="bg-slate-100 hover:bg-red-600 hover:text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-all inline-flex items-center gap-1"
                      >
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full border border-slate-200 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleSaveRate} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-extrabold text-slate-700">السعر الأساسي (حتى 3 كجم):</label>
                <input
                  type="number"
                  required
              ...
              <div className="space-y-1">
                <label className="text-xs font-extrabold text-slate-700">سعر الكيلو الإضافي (ج.م):</label>
                <input
                  type="number"
                  required
