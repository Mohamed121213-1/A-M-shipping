...
export const CourierNotificationToast: React.FC<CourierNotificationToastProps> = ({
  ...
  return (
    <div className="fixed top-5 left-1/2 -translate-x-1/2 md:left-auto md:right-6 md:translate-x-0 z-[100] max-w-md w-[92vw] sm:w-[420px] bg-slate-900 text-white rounded-2xl shadow-2xl border-2 border-amber-500 overflow-hidden animate-in slide-in-from-top-5 duration-300">
      {/* Top Banner */}
      <div className={`px-4 py-2 flex items-center justify-between text-xs font-bold text-white ${
        ...
        <span className="text-[10px] bg-black/20 px-2 py-0.5 rounded font-mono shrink-0 ml-1">{notification.timestamp}</span>
      </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-start justify-between gap-2 border-b border-slate-800 pb-2.5">
          ...
          <div className="text-left">
            ...
            <span className="font-mono font-black text-xs text-amber-400 bg-amber-950/80 px-2 py-1 rounded border border-amber-800/80 block mt-0.5">
        ...
        {notification.statusNote && (
          <div className="bg-slate-800/90 p-2.5 rounded-xl border border-slate-700 text-xs text-amber-200 font-medium leading-relaxed">
            <span className="font-bold text-amber-400 block mb-0.5">📝 تفاصيل الإشعار:</span>
            {notification.statusNote}
          </div>
        )}
      ...
      <div className="w-full bg-slate-800 h-1 overflow-hidden">
        <div className="bg-gradient-to-r from-amber-500 to-emerald-500 h-full w-full animate-shrink-width" style={{ animationDuration: '10s' }} />
      </div>
    </div>
  );
};
